...
const Home = ( {db, isLoggedIn, user } ) => {
    ...
    const chat = messages.map(msg => {
        return ( 
            <li key={msg.id} ref={scrollRef} className={`msg-container ${user && msg.author === user.email ? "active" : ""}`}>
    ...
    return ( 
        <div className="chat-container">
            ...
            {isLoggedIn && 
                ...
                <ul>{chat}</ul>
                    <form>
                        <input type="text" className="chat-input" id="message" name="message" value={message} onChange={handleMessageChange} />
